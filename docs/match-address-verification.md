...
| 役割 | マシン | IP | 備考 |
|------|--------|-----|------|
| 接続元（クライアント） | Mac M1 Pro | `192.168.0.84` | `ssh-pushkey` 実行側 |
| 接続先（サーバ） | OMEN16-WIN | `192.168.0.100` | Windows 11 + OpenSSH |
...
|-------------|------------------------|--------|
| `192.168.0.84`（実IP） | `Match Address` | `.ssh/authorized_keys` |
...
# 新コード相当（実IP）
& $sshd -T -C "user=pushadmin,host=client,addr=192.168.0.84" `
  -f C:\ProgramData\ssh\sshd_config | Select-String AuthorizedKeysFile
```
...
| `192.168.0.84` | `.ssh/authorized_keys` |
...
```bash
# Mac 側
ssh-pushkey pushadmin@192.168.0.100
```
...
Mac の実IP（`192.168.0.84`）は `Match Address` にマッチするので
...
./ssh-pushkey-dev pushadmin@192.168.0.100
```

...
- [ ] 方針決定（A / B / 折衷）
- [ ] 決定に応じて `ssh.go` の `createHostKeyCallback` 変更 + テスト

#### B. `~/.ssh/config` 非解決（機能・既知の制約）

**現状**: `dialSSH` が `ClientConfig` を素から構築するため、`ssh-pushkey myalias` の Host alias / 個別 User・Port・ProxyJump が効かない（`ssh.go:35`）。クライアントは Linux/Mac/WSL 上で動くため実害のある機能ギャップ。「自己完結ダイヤラ」設計の裏返しでもある。

- [ ] 方針決定（完全対応はスコープ大 / 最小対応として README に制約明記 + ProxyJump 等は非対応のまま）
- [ ] 決定に応じて対応

...
- [x] 鍵 blob（type + base64 部分）のみで比較するように変更
  > Go 側で `pubKeyBlob()`（`ssh.ParseAuthorizedKey` → `ssh.MarshalAuthorizedKey`）が blob を抽出し `buildDeployScript` に渡す。PS 側は `Select-String` を廃止し、行ごとに先頭2フィールドを `-ceq`（base64 は大小区別が必要）で blob 比較。空行・`#` 行は skip。
  > 既知の許容: `command="..." ssh-rsa ...` の options 前置行は検知漏れし得るが最悪「重複追記」のみで破損なし（ssh-pushkey 自身の鍵では発生しない）。
- [x] テスト追加（`TestPubKeyBlob` / `_Invalid`、`deploy_test.go` の生成スクリプト検証を `-ceq $keyBlob` に更新）

...
- 実装・実機検証完了。PR #13 / MR #16（Draft）。実機（127.0.0.1）で SSH_CONNECTION 伝播・CLIXML 耐性・
  ...
  > 非 loopback クライアント（Mac `192.168.0.84` → Windows `192.168.0.100`）から `Match Address`
...
- [x] 事前確認: `sshd.exe -T -C "addr=127.0.0.1"` と `addr=192.168.0.84` で `AuthorizedKeysFile` が

...
1. **Windows 側で専用テストアカウント2つを作成**（管理者 PowerShell）:
   ```powershell
   # 管理者アカウント
   $pw1 = Read-Host -AsSecureString "test-pushkey-admin のパスワード"
   New-LocalUser -Name "test-pushkey-admin" -Password $pw1 `
     -FullName "ssh-pushkey integration (admin)" -Description "loopback-only test account"
   Add-LocalGroupMember -Group "Administrators" -Member "test-pushkey-admin"

   # 一般アカウント（Administrators には入れない）
   $pw2 = Read-Host -AsSecureString "test-pushkey-user のパスワード"
   New-LocalUser -Name "test-pushkey-user" -Password $pw2 `
...
2. **認証情報ファイルを作成**（いずれも gitignore 済み・平文保存だが loopback 専用の使い捨てアカウント）:
   - `.env.integration`（管理者）:
     ...
     export SSH_TEST_USER=test-pushkey-admin
     export SSH_TEST_PASSWORD='手順1のパスワード'
     ```
   - `.env.integration.user`（一般）: 上記の USER/PASSWORD を `test-pushkey-user` 用に。
...
- 実装・実機検証完了。PR #13 / MR #16（Draft）。実機（127.0.0.1）で SSH_CONNECTION 伝播・CLIXML 耐性・
  ...
  > **`addr=` の未検証ギャップは Issue #17 で独立追跡**（非 loopback クライアントでの Match Address
  > 誤判定解消の e2e 検証）。loopback では `addr=127.0.0.1` が旧固定値と機能的に同一のため本来の
  > バグシナリオを exercise できない。Issue #15（`host=` 逆引き）とは別物。
  > **両テストアカウント整備後の追加検証アイデア**: 一般アカウントを `Match User test-pushkey-user`
  > 等で別 AuthorizedKeysFile に振る sshd_config を用意すれば、loopback でも Match 評価の差を観測できる
  > 可能性がある（今後の検討）。
